...
output_attribute_list(output::pointer ofp, attributes_list_ty *alp)
{
    ...
    for (size_t j = 0; j < alp->length; ++j)
    {
        ...
        //
        // Do not output attributes representing the
        // history_get_command, the value may be wrong on the other side.
        //
        if (name == HISTORY_GET_COMMAND)
            continue;
...
revml_send(void)
{
    ...
    if (mime_header)
    {
        ...
        nstring s = project_name_get(cid.get_pp());
        nstring s2 = cid.get_cp()->brief_description_get();
        if (entire_source)
    ...
    //
    emit_time(ofp, "time", cid.get_cp()->completion_timestamp());
    ...
    if (entire_source)
    {
        ...
        //
        time_t limit = cid.get_cp()->completion_timestamp();
    ...
    for (size_t j = 0;; ++j)
    {
        ...
        case file_usage_manual_test:
            ...
            case file_action_modify:
                ...
                {
                    string_ty *s = cid.get_cp()->file_path(src_data->file_name);
                    assert(s);
                    if (s)
                    {
                        os_become_orig();
    ...
    for (size_t m = 0; m < file_list->length; ++m)
    {
        ...
        {
            nstring s = cid.get_cp()->version_get();
            ofp->fprintf("<rev id=\"%s\">\n", s.c_str());
        }
...
#endif
                {
                    string_ty *s = cid.get_cp()->file_path(csrc->file_name);
                    if (!s)
