...
#include <aecomplete/complete/project/reviewer.h>
...
{
    complete_project_reviewer_ty *this_thing;

    this_thing = (complete_project_reviewer_ty *)cp;
...
perform(complete_ty *cp, shell_ty *sh)
{
    ...
    for (j = 0; ; ++j)
    {
	name = project_reviewer_nth(this_thing->pp, j);
	if (!name)
	    break;
...
complete_ty *
complete_project_reviewer(project_ty *pp)
{
    complete_ty     *result;
    complete_project_reviewer_ty *this_thing;

    result = complete_new(&vtbl);
    this_thing = (complete_project_reviewer_ty *)result;
    this_thing->pp = pp;
    return result;
}
