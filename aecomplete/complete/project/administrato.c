...
#include <complete/project/administrato.h>
#include <project.h>
#include <project/history.h>
#include <shell.h>


typedef struct complete_project_administrat_ty complete_project_administrat_ty;
...
destructor(cp)
    complete_ty     *cp;
{
    complete_project_administrat_ty *this;

    this = (complete_project_administrat_ty *)cp;
    ...
    shell_ty        *sh;
{
    ...
    for (j = 0; ; ++j)
    {
	name = project_administrator_nth(this->pp, j);
	if (!name)
	    break;
...
complete_ty *
complete_project_administrator(pp)
    project_ty      *pp;
{
    complete_ty     *result;
    complete_project_administrat_ty *this;
    ...
    this = (complete_project_administrat_ty *)result;
    this->pp = pp;
    return result;
}
