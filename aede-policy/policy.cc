...
//
// MANIFEST: implementation of the policy class
//
...
policy()
{
	...
	case arglex_token_trunk:
    	    change.command_line_parse(usage);
	    continue;
    ...
    //
    if (!change_is_being_developed(change.get_cp()))
	change_fatal(change.get_cp(), 0, i18n("bad de state"));

    //
    // Perform all of the validations.
    // If any of them fail, quit with a failure exit status.
    //
    bool ok = to_do.run(change.get_cp());
    if (!ok)
	quit(1);
    trace(("}\n"));
}
