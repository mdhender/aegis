...
validation_authors::~validation_authors()
{
}
...
validation::pointer
validation_authors::create(void)
{
    return pointer(new validation_authors());
}
...
bool
validation_authors::run(change::pointer cp)
{
    //
    // Don't check branches, only individual changes have control over
    // the AUTHORS source file.
    //
    if (cp->was_a_branch())
        return true;
    ...
    //
    fstate_src_ty *src = cp->file_find("AUTHORS", view_path_extreme);
    if (!src)
    {
        sub_context_ty sc;
        sc.var_set_charstar("File_Name", "AUTHORS");
