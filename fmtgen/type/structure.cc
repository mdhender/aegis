...
gen_code(type_ty *type)
{
	...
	ep = &this_thing->element[j];
	type_gen_code_declarator(ep->type, ep->name, 0, ep->attributes);
    }
    if (!this_thing->toplevel)
    {
	indent_printf("output_fputs(fp, \"}\");\n");
	...
	ep = &this_thing->element[j];
	type_gen_code_call_xml(ep->type, ep->name, ep->name, ep->attributes);
    }
    if (!this_thing->toplevel)
    {
	indent_printf("output_fputs(fp, \"</\");\n");
	...
	redef = !!(ep->attributes & ATTRIBUTE_REDEFINITION_OK);
	indent_printf
	(
	    "%d, /* redefinition %s ok */\n",
	    redef,
