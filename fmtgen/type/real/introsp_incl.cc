...
type_real_introspector_include::~type_real_introspector_include()
{
    trace(("%s\n", __PRETTY_FUNCTION__));
}
...
type::pointer
type_real_introspector_include::create(generator *gen)
{
    trace(("%s\n", __PRETTY_FUNCTION__));
    return pointer(new type_real_introspector_include(gen));
}


void
type_real_introspector_include::gen_body()
    const
{
...
void
type_real_introspector_include::gen_declarator(const nstring &variable_name,
    bool is_a_list, int, const nstring &cmnt) const
{
    ...
    (
        ...
        "0.0 if it has not been set.\n"
    );
    printf("*/\n");
    printf("double %s_get() const;\n", variable_name.c_str());
