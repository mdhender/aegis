    ...
    string_list_ty *modifier)
{
	...
	case cstate_state_being_integrated:
		 ...
		 */
		if (!change_file_find(cp, filename))
		    break;
	    }
		...
		if (all && !master_cp->bogus)
		{
		     ...
		     */
		    if (!change_file_find(master_cp, src->file_name))
			continue;
		}
		++used;
		 ...
		 */
		clp = symtab_query(stp, src->file_name);
		if (!clp)
		{
		    clp = change_list_new();
	...
	clp = symtab_query(stp, the_file_name);
	assert(clp);
	if (!clp)
	    continue;
	...
	for (k = 0; k < clp->length; ++k)
	{
	    ...
	    cp = clp->item[k];
	    src = change_file_find(cp, the_file_name);
	    assert(src);
	    if (!src)
		continue;
	    cstate_data = change_cstate_get(cp);
