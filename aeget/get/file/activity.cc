    ...
    string_list_ty *modifier)
{
    ...
    for (j = 0; ; ++j)
    {
        ...
        case cstate_state_being_integrated:
            if (!all)
            {
                ...
                //
                if (!cp->file_find(nstring(filename), view_path_first))
                    break;
            }
    ...
    for (j = 0; j < key.nstrings; ++j)
    {
        ...
        for (k = 0; k < clp->length; ++k)
        {
            ...
            cp = clp->item[k];
            src = cp->file_find(nstring(the_file_name), view_path_first);
            assert(src);
            if (!src)
                continue;
            cstate_data = cp->cstate_get();
            ...
            {
                printf("<br>\n<i>");
                html_encode_string(cp->developer_name());
                printf("</i>");
            }
            if (cstate_data->state == cstate_state_being_integrated)
            {
                printf("<br>\n<i>");
                html_encode_string(cp->integrator_name());
                printf("</i>");
            }
            printf("</td>\n<td valign=\"top\">\n");
