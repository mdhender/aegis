...
emit_path(fstate_src_ty *src, string_ty *from)
{
    ...
    {
        nstring s(ap->value);
        s = http_sanitize_content_type(s);
    ...
    string_list_ty *modifier)
{
    ...
    for (j = 0; j < gizzards.nstrings; ++j)
    {
        ...
        (
            !cp->bogus
        &&
            0 != (src = cp->file_find(nstring(s), view_path_first))
        )
        {
            is_a_change_file = 1;
            is_a_file = true;
        }
        else if (0 != (src = cp->pp->file_find(s, view_path_simple)))
        {
            is_a_project_file = 1;
            is_a_file = true;
        }
    ...
    string_list_ty *modifier)
{
    ...
    if (!cp->bogus)
    {
        ...
        (
            cp->is_completed()
        &&
            cp->pp->file_find(filename, view_path_simple)
        )
        {
            file_event   *fep;
            string_ty       *s;
