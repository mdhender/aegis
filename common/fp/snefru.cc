    ...
    int level)
{
    ...
    B14 = input[14];
    B15 = input[15];
    for (idx = 0; idx < 2 * level; idx += 2)
    {
	//
	// set up the base address for the two S-box pointers.
	//
	SBox0 = snefru512_rotated[0][idx];
	SBox1 = SBox0 + 256;
	//
...
snefru_sum(fingerprint_ty *p, char *obuf, size_t obuf_len)
{
	...
	if (j && !(j & 3))
    	    *cp++ = ' ';
	snprintf(cp, obuf + obuf_len - cp, "%2.2X", h[j]);
	cp += strlen(cp);
    }
    *cp = 0;
}
