...
main(int argc, char **argv)
{
    ...
    for (size_t j = 0; ; ++j)
    {
        ...
        // Change files supercede project files,
        // so we hide them.
        fstate_src_ty *csrc = cid.get_cp()->file_find(src, view_path_first);
        if (csrc)
            continue;

        if ((action & (1 << src->action)) && (usage & (1 << src->usage)))
            names.push_back(nstring(src->file_name));
    }
    for (size_t j = 0; ; ++j)
    {
    ...
    //
    // Emit the file names, sorted.
    //
    names.sort();
    for (size_t j = 0; j < names.size(); ++j)
    {
        nstring filename = names[j];

        // resolve the file names, if asked
        if (resolve)
        {
            filename =
                nstring(change_file_source(cid.get_cp(), filename.get_ref()));
            if (filename.empty())
                continue;
        }

        // quote the file names, if asked
        switch (quote)
        {
        default:
        case quote_none:
