...
static string_ty *first_line _((string_ty *));
...
static int extract_year _((time_t));
...
static cstate_history change_history_fake _((change_ty *, string_ty *, time_t));

static cstate_history
change_history_fake(cp, who, when)
	change_ty	*cp;
	string_ty	*who;
	time_t		when;
{
	...
	change_set_ty	*csp;
{
	...
	for (j = 0; j < csp->file.length; ++j)
	{
		...
		p_src_data = project_file_find(pp, csfp->filename);
		if (!p_src_data)
