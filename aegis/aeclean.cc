    ...
    const struct stat *)
{
    ...
    case dir_walk_symlink:
        ...
        {
            fstate_src_ty *psrc = sip->cp->pp->file_find(s1, view_path_extreme);
    ...
    case dir_walk_file:
        ...
        {
            fstate_src_ty *csrc =
                sip->cp->file_find(nstring(s1), view_path_first);
            if (csrc && csrc->usage != file_usage_build)
            {
                delete_me = false;
                touch_me = sip->touch;
            }
        ...
        &&
            sip->minimum
        &&
            sip->cp->pp->file_find(s1, view_path_extreme)
        )
            delete_me = false;
...
clean_main(void)
{
    ...
    for (j = 0;; ++j)
    {
        ...
        // build various paths
        //
        path = cp->file_path(c_src_data->file_name);
        assert(path);
    ...
    //
    // Run the develop begin early command, because we have probably
    // removed its side effects.  This is specifically run INside
    // the locks.
    //
