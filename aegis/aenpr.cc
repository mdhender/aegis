...
static void
new_administrator_add(project *pp, user_ty::pointer candidate)
{
    assert(pp);
    assert(candidate);
    ...
    for (size_t j = 0; j < wlp.size(); ++j)
    {
        ...
        project *branch = project_alloc(wlp.string[j]);
...
new_project_main(void)
{
    ...
    while (arglex_token != arglex_token_eoln)
    {
        ...
        case arglex_token_directory:
            ...
            //
            home = nstring(arglex_value.alv_string);
            break;
    ...
    if (home.empty())
    {
        ...
        os_become_undo();
        if (project_name_get(pp).length() > (size_t)name_max)
            fatal_project_name_too_long
            (
                project_name_get(pp).get_ref(),
                name_max
            );
        home = os_path_join(s2, project_name_get(pp));
