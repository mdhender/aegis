...
make_transparent_undo_usage(void)
{
    ...
    (
	stderr,
	"usage: %s -Make_Transparent_Undo [ <option>... ] <filename>...\n",
	progname
    );
    fprintf
    (
	stderr,
	"       %s -Make_Transparent_Undo -List [ <option>... ]\n",
	progname
    );
    fprintf(stderr, "       %s -Make_Transparent_Undo -Help\n", progname);
    quit(1);
}


static void
make_transparent_undo_help(void)
{
    help("aemtu", make_transparent_undo_usage);
}
...
static void
make_transparent_undo_main(void)
{
    sub_context_ty  *scp;
    size_t	    j;
    size_t	    k;
    ...
    trace(("make_transparent_undo_main()\n{\n"));
    arglex();
    string_list_ty wl;
    project_name = 0;
    change_number = 0;
	...
	(
	    cp,
	    scp,
	    i18n("make transparent undo $filename complete")
	);
	sub_context_delete(scp);
    }
...
make_transparent_undo(void)
{
    ...
    arglex_dispatch(dispatch, SIZEOF(dispatch), make_transparent_undo_main);
    trace(("}\n"));
}
