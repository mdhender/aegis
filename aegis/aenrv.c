...
static void new_reviewer_list _((void (*)(void)));

static void
new_reviewer_list(usage)
	void	(*usage)_((void));
{
...
new_reviewer_main()
{
	...
	while (arglex_token != arglex_token_eoln)
	{
		...
		case arglex_token_project:
			...
			if (project_name)
				duplicate_option_by_name(arglex_token_project, new_reviewer_usage);
			project_name = str_from_c(arglex_value.alv_string);
			break;
...
new_reviewer()
{
	...
	case arglex_token_list:
		new_reviewer_list(new_reviewer_usage);
		break;
	}
	trace((/*{*/"}\n"));
}
