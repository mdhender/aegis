    ...
    long cn2, log_style_ty log_style, string_list_ty *wl)
{
    ...
    if (!wl->nstrings)
    {
        for (j = 0;; ++j)
        {
            ...
            src2_data =
                acp->file_find(nstring(src1_data->file_name), view_path_first);
            if (!src2_data)
                continue;
    ...
    else
    {
        ...
        acp->search_path_get(&search_path2, true);
        ...
        *wl = wl2;
        ...
        for (j = 0; j < wl->nstrings; ++j)
        {
            ...
            s1 = wl->string[j];
            src1_data = cp->file_find(nstring(s1), view_path_first);
            if (!src1_data)
            {
                sub_context_ty  *scp;
            ...
            src2_data = acp->file_find(nstring(s1), view_path_first);
            if (!src2_data)
            {
                sub_context_ty  *scp;
    ...
    for (j = 0; j < wl->nstrings; ++j)
    {
        ...
        assert(src1_data);
#ifdef DEBUG
        fstate_src_ty *src2_data = acp->file_find(nstring(s1), view_path_first);
        assert(src2_data);
#endif

        most_recent = acp->file_path(src1_data);
        assert(most_recent);
        if (change_has_merge_command(cp))
        {
            output_filename = cp->file_path(src1_data);
...
difference_main(void)
{
    ...
    //
    // Take a lock on the appropriate row of the change table.
    //
    ...
    else
    {
        ...
        for (j = 0; j < wl.nstrings; ++j)
        {
            ...
            else if (cp->file_find(nstring(s1), view_path_first))
                wl2.push_back_unique(s1);
            else
            {
                sub_context_ty  *scp;
    ...
    for (j = 0; j < wl.nstrings; ++j)
    {
        ...
        //
        s1 = wl.string[j];
        fstate_src_ty *src1_data = cp->file_find(nstring(s1), view_path_first);
        assert(src1_data);
        trace(("src1_data->file_name = \"%s\"\n",
        ...
        //
        fstate_src_ty *src2_data = pp2->file_find(src1_data, view_path_extreme);
        if (!src2_data)
            continue;
        if (change_file_up_to_date(pp2, src1_data))
            continue;
    ...
    if (merge_select == uconf_diff_preference_only_merge)
    {
        ...
        for (j = 0; j < wl.nstrings; ++j)
        {
            ...
            //
            fstate_src_ty *src2_data =
                pp2->file_find(src1_data, view_path_extreme);
            if (!src2_data)
            {
                //
            ...
            if (change_has_merge_command(cp))
            {
                outname = cp->file_path(src1_data);
            ...
            {
                curfile = cp->file_path(src1_data);
                trace_string(curfile->str_text);
            ...
            p_src_data = cp->pp->file_find(src1_data, view_path_extreme);
            if (!p_src_data)
            {
                //
            ...
            //
            // If necessary, update the UUID.
            //
            if (!str_equal(src1_data->uuid, src2_data->uuid))
            {
                if (src1_data->uuid)
                {
                    str_free(src1_data->uuid);
                    src1_data->uuid = 0;
                }
                if (src2_data->uuid)
    ...
    else
    {
        ...
        for (j = 0; j < wl.nstrings; ++j)
        {
            ...
            trace(("src1_data->file_name = \"%s\"\n",
                src1_data->file_name->str_text));
            trace(("src1_data->action = %s\n",
                file_action_ename(src1_data->action)));
            trace(("src1_data->usage = %s\n",
                file_usage_ename(src1_data->usage)));
            trace(("src1_data->uuid = %s\n",
                src1_data->uuid ? src1_data->uuid->str_text : "NULL"));

            //
            // locate the equivalent project file
            //
            fstate_src_ty *src2_data =
                pp2bl->file_find(src1_data, view_path_extreme);
            trace(("src2_data = %08lX\n", (long)src2_data));
            if (src2_data)
            {
                trace(("src2_data->action2 = %s\n",
                    file_action_ename(src2_data->action)));
                trace(("src2_data->usage2 = %s\n",
                    file_usage_ename(src2_data->usage)));
                trace(("src2_data->file_name2 = \"%s\";\n",
                    src2_data->file_name->str_text));
                trace(("src2_data->uuid = %s\n",
                    src2_data->uuid ? src2_data->uuid->str_text : "NULL"));
            }
            ...
            case file_action_remove:
                ...
                (
                    src1_data->move
                &&
                    cp->file_find(nstring(src1_data->move), view_path_first)
                )
                    continue;
                break;
            ...
            case file_action_create:
                ...
                pseudo_create:
                {
                    ...
                    (
                        src1_data->move
                    &&
                        pp2bl->file_exists(nstring(src1_data->move))
                    )
                    {
            ...
            case file_action_remove:
                ...
                //
                {
                    ...
                    (
                        src1_data->move
                    &&
                        cp->file_exists(nstring(src1_data->move))
                    )
                    {
                        //
                        // Use src2_data to handle correctly the case
                        ...
                        //
                        assert(src2_data);
                        input_file_name = cp->file_path(src2_data);
            ...
            case file_action_modify:
                ...
                //
                {
                    fstate_src_ty *pp2src_data =
                        pp2bl->file_find(s1, view_path_extreme);
                    if (!pp2src_data)
                    {
