...
static rpt_value_ty *list_evaluate _((rpt_expr_ty *));
...
static rpt_value_ty *comma_evaluate _((rpt_expr_ty *));
...
{
	assert(ep->nchild == 2);
	rpt_value_free(rpt_expr_evaluate(ep->child[0]));
	return rpt_expr_evaluate(ep->child[1]);
}
