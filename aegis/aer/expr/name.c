	...
	if (!data)
	{
		...
		if (!name2)
		{
			data =
				rpt_value_error
				(
					"the name \"%S\" is undefined",
					name
				);
			report_error("%S", rpt_value_error_query(data));
		}
		else
		{
			report_error
			(
	      "the name \"%S\" is undefined, guessing you meant \"%S\" instead",
				name,
				name2
			);
	...
	string_ty	*name;
{
	...
	{
		report_error("the name \"%S\" has already been used", name);
		return;
	}
