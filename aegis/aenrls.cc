...
new_release_main(void)
{
    ...
    project      *version_pp[SIZEOF(new_version_number)];
    ...
    project_version_previous_set(pp[1], project_version_get(pp[0]).get_ref());
    ...
    //
    project_description_set(pp[1], project_description_get(pp[0]).get_ref());
    ...
    for (j = 0;; ++j)
    {
        ...
        if (!c_src_data)
            break;
        p_src_data =
            ppp->file_find(c_src_data->file_name, view_path_none);
        assert(p_src_data);
    ...
    //
    // Add the change to the branch's history.
    // Do not set the change UUID.
    //
    project_history_new
    (
        ppp,
