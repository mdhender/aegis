...
finish_merge(change_identifier &cid)
{
    ...
    for (size_t j = 0; ; ++j)
    {
        ...
        //
        fstate_src_ty *src2_data = pp->file_find(src1_data, view_path_extreme);
        if (!src2_data)
            continue;
        ...
        nstring command = "aegis --diff --merge-only --project="
            ...
            + nstring::format("%ld", cid.get_change_number())
            + " --verbose";
        command += nstring(trace_args());
        int flags = 0;
...
finish_diff(change_identifier &cid)
{
    ...
    for (size_t j = 0; ; ++j)
    {
        ...
        // build various paths
        //
        string_ty *path = cp->file_path(s1);
        assert(path);
...
calculate_youngest_file(change_identifier &cid)
{
    ...
    for (size_t j = 0;; ++j)
    {
        ...
        string_ty *path = cp->file_path(c_src_data->file_name);
...
static void
begin_integration(change_identifier &cid)
{
    project *pp = cid.get_pp();
    nstring dir = nstring(pp->trunk_get()->home_path_get());
    nstring command = "aegis --integrate-begin --project="
        + nstring(project_name_get(pp)).quote_shell() + " --change="
        + nstring::format("%ld", cid.get_change_number())
        + " --verbose";
    if (minimum_option)
