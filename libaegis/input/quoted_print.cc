...
//      Copyright (C) 2001-2006, 2008, 2009, 2012 Peter Miller
//
...
input
input_quoted_printable::create(input &arg)
{
    return new input_quoted_printable(arg);
}
...
bool
input_quoted_printable::recognise(input &deeper)
{
    nstring_accumulator sa;
    // state 0: waiting for '='
    // state 1: seen "=", waiting for "\r\n" or "\n" or "XX"
    // state 2: seen "=\r", waiting for "\n"
    // state 3: seen "=X", waiting for X
    int state = 0;
    bool seen_eq = false;
    while (sa.size() < 8000)
    {
        ...
        {
            fail:
            deeper->unread(sa.get_data(), sa.size());
            return false;
        }
        switch (state)
        {
    ...
    //
    // We can't definitively say is is quoted-printable is we never saw
    // a single '=' symbol.  Given that we care about patches, and they
    // tend to be code, there should be a '=' symbol in there somewhere.
    //
    return seen_eq;
}
