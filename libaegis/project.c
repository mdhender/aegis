    ...
    project_ty	    *pp;
{
	...
	pstate_data->copyright_years = pstate_copyright_years_list_type.alloc();
    ...
    project_ty	    *pp;
{
	...
	pp->changes_path =
	    ...
	    (
		"%S.branch",
		project_change_path_get(pp->parent, pp->parent_bn)
	    );
	return;
    }
    ...
    change_ty	    *cp;
{
    ...
    /* punctuation? */
    project_name =
	str_format("%S.%ld", ppp->name, magic_zero_decode(cp->number));
    pp = project_alloc(project_name);
    pp->parent = project_copy(ppp);
    pp->parent_bn = cp->number;
    pp->pcp = cp;
    pp->changes_path =
	str_format("%S.branch", project_change_path_get(ppp, cp->number));
    trace(("return %8.8lX;\n", (long)pp));
    trace(("}\n"));
    return pp;
}
    ...
    project_ty	    *pp;
{
    ...
    if (!pp->info_path)
    {
	pp->info_path = str_format("%S/info", project_home_path_get(pp));
    }
    ...
    project_ty	    *pp;
{
    ...
    if (!pp->changes_path)
    {
	pp->changes_path = str_format("%S/change", project_info_path_get(pp));
    }
    ...
    project_ty	    *pp;
{
    ...
    if (!pp->pstate_path)
    {
	pp->pstate_path = str_format("%S/state", project_info_path_get(pp));
    }
    ...
    project_ty	    *pp;
{
    ...
    if (!pp->history_path)
    {
	pp->history_path = str_format("%S/history", project_home_path_get(pp));
    }
    ...
    project_ty	    *pp;
{
	...
	if (s->str_length)
	{
	    ...
	    /* ...punctuation? */
	    s2 = str_format("%S.%ld", s, magic_zero_decode(pp->parent_bn));
	    str_free(s);
	    s = s2;
	}
	else
