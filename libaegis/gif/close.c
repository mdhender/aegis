...
output(fp, fn, code, code_size)
	...
	int		code_size;
{
	trace(("output(fp, fn = %8.8lX, code = 0x%.*X, code_size = %d)", fp, fn, (code_size + 3) / 4, code, code_size));
	...
	int		init_bits;
{
	...
	trace(("write_image(fp, fn = %8.8lX, data = %8.8lX, width = %d, height = %d, init_bits = %d)", fp, fn, data, width, height, init_bits));
	...
	for (;;)
	{
		...
		if (code_cur != IMPOSSIBLE)
		{
			...
			trace(("invert_pos = %d; code_next = 0x%03X;", invert_pos, code_next));
		...
		if (code_cur != IMPOSSIBLE)
		{
			...
			if (state == state_normal && table_pos < (1 << MAX_BITS))
			{
				...
				trace(("invert[invert_pos = %d] = 0x%03X;", invert_pos, table_pos));
				++table_pos;
			}
			state = state_normal;
