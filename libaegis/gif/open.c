...
static void format_error _((char *));
...
static int gif_getc _((FILE *, char *));
...
static void read_image _((FILE *fp, char *fn, int row_bytes,
	unsigned char *data, int width, int height, int interlaced));

static void
read_image(fp, fn, row_bytes, data, width, height, interlaced)
	FILE		*fp;
	char		*fn;
	int		row_bytes;
	unsigned char	*data;
	...
	int		interlaced;
{
	...
	if (y != height)
	{
		trace(("too few pixels (y = %d, height, x = %d, width = %d)", y, height, x, width));
		format_error(fn);
	}
#endif
...
static int get_le_short _((FILE *, char *));
	...
	int		mode;
{
	...
	if (!result->width || result->width > SIZE_MAX)
		format_error(fn);
	result->height = get_le_short(fp, fn);
	if (!result->height || result->height > SIZE_MAX)
		format_error(fn);
		...
		case ',':
			{
				...
				(
					...
					fn,
					result->width,
					result->image_flat + image_left +
						image_top * result->width,
					image_width,
					image_height,
					interlaced
				);
