...
input_ty::~input_ty()
{
    trace(("input_ty::~input_ty(this = %p)\n{\n", this));
...
{
    trace(("input_ty::reference_count_up(this = %p)\n{\n", this));
    assert(reference_count_valid());
    reference_count++;
...
{
    trace(("input_ty::reference_count_down(this = %p)\n{\n", this));
...
input_ty::getc_complicated()
{
    ...
    // the new buffer.
    //
    ssize_t nbytes = read_inner(buffer, buffer_size);
    if (nbytes <= 0)
        return (-1);
    buffer_position = buffer;
