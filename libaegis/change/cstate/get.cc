...
cstate_to_fstate(change::pointer cp)
{
    ...
    cp->fstate_stp = new symtab_ty(5);
    cp->fstate_uuid_stp = new symtab_ty(5);
    ...
    for (size_t j = 0; j < cp->cstate_data->src->length; ++j)
    {
        ...
        *addr_p = src2;
        ...
        // index to track also
        //
        cp->fstate_stp->assign(src2->file_name, src2);
        if (src2->uuid && (!src2->move || src2->action != file_action_remove))
            cp->fstate_uuid_stp->assign(src2->uuid, src2);
    }
...
change::cstate_get()
{
    trace(("change::cstate_get(this = %p)\n{\n", this));
    assert(reference_count >= 1);
    change_lock_sync(this);
    if (!cstate_data)
    {
        string_ty *fn = cstate_filename_get();
        change_become(this);
