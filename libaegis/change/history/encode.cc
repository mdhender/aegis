...
change_history_encode(change::pointer cp, fstate_src_ty *src, int *unlink_p)
{
    ...
    trace(("change_history_encode(cp = %p, src->file_name = \"%s\")\n{\n",
    ...
    *unlink_p = 1;
    ...
    case pconf_history_content_limitation_binary_capable:
        ...
        *unlink_p = 0;
        trace(("}\n"));
        return cp->file_path(src->file_name);
    ...
    output::pointer op;
    {
        ...
        output::pointer op1x =
            output_filter_quoted_printable::create(op1, min_qp_enc);

        output::pointer op2 = output_file::text_open(ofn2);
        output::pointer op2x = output_filter_base64::create(op2);
    ...
    if (src->uuid)
        os_unlink(ofn3);
    str_free(ofn3);
    off_t size1 = os_file_size(ofn1);
    off_t size2 = os_file_size(ofn2);
    if (size1 <= size2)
    {
