...
//
// MANIFEST: implementation of the change_history_tran_end_com class
//
...
void
change_run_history_transaction_end_command(change::pointer cp)
{
    trace(("change_run_history_transaction_end_command(cp = %8.8lX)\n{\n",
	(long)cp));
    assert(cp->reference_count >= 1);
    pconf_ty *pconf_data = change_pconf_get(cp, 1);
    string_ty *the_command = pconf_data->history_transaction_end_command;
