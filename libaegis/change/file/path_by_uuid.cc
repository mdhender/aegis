...
string_ty *
change::file_path_by_uuid(string_ty *uuid)
{
    trace(("change::file_path_by_uuid(this = %p, uuid = \"%s\")\n{\n",
        this, uuid->str_text));
    fstate_get();
    assert(fstate_stp);
    string_ty *result = 0;
...
nstring
change::file_path_by_uuid(const nstring &uuid)
{
    string_ty *result = file_path_by_uuid(uuid.get_ref());
    if (!result)
        return nstring();
