...
{
    trace(("painful_development_directory_style(cp = %p)\n{\n", cp));
    pconf_ty *pconf_data = change_pconf_get(cp, 0);
    bool result =
        (
            pconf_data
...
{
    trace(("change_file_template(cp = %p, filename = \"%s\", ip = %p, "
        "use_template = %d)\n{\n", cp, filename->str_text, up.get(),
        use_template));
    ...
    else
    {
        ...
        &&
            cp->pp->file_find(filename, view_path_extreme)
        )
        {
            //
