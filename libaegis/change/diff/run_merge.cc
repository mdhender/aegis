...
//
// MANIFEST: functions to manipulate run_merges
//
...
void
change_run_merge_command(change_ty *cp, user_ty *up, string_ty *original,
    string_ty *most_recent, string_ty *input, string_ty *output)
{
    ...
    trace
    ((
	"change_run_merge_command(cp = %8.8lX, up = %8.8lX, original = \"%s\", "
	    "most_recent = \"%s\", input = \"%s\", output = \"%s\")\n{\n",
	(long)cp,
	(long)up,
	original->str_text,
	...
	sub_var_set_charstar(scp2, "FieLD_Name", "merge_command");
	change_fatal
	(
	    cp,
	    scp2,
