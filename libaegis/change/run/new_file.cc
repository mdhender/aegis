...
void
change::run_new_file_command(string_list_ty *wlp, const user_ty::pointer &up)
{
    trace(("change::run_change_file_command(this = %08lX)\n{\n",
        (long)this));
    assert(!lock_active());
    assert(reference_count >= 1);
    pconf_ty *pcp = change_pconf_get(this, 0);
    string_ty *the_command = pcp->new_file_command;
    if (!the_command)
    ...
    str_free(the_files);
    the_command = substitute(&sc, this, the_command);
