...
//
// MANIFEST: functions to manipulate maketranundos
//
...
void
change_run_make_transparent_undo_command(change_ty *cp, string_list_ty *wlp,
    user_ty *up)
{
    sub_context_ty  *scp;
    pconf_ty        *pconf_data;
    ...
    trace(("change_run_make_transparent_undo_command(cp = %08lX)\n{\n",
	(long)cp));
    assert(cp->reference_count >= 1);
    pconf_data = change_pconf_get(cp, 0);
    the_command = pconf_data->make_transparent_undo_command;
    if (!the_command)
	the_command = pconf_data->change_file_undo_command;
    if (!the_command)
