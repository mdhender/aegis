...
void
change_run_copy_file_command(change_ty *cp, string_list_ty *wlp, user_ty *up)
{
    sub_context_ty  *scp;
    pconf_ty        *pconf_data;
    ...
    trace(("change_run_copy_file_command(cp = %08lX)\n{\n", (long)cp));
    assert(cp->reference_count >= 1);
    pconf_data = change_pconf_get(cp, 0);
    the_command = pconf_data->copy_file_command;
    if (!the_command)
