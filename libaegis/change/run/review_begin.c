 ...
 *
 * MANIFEST: functions to manipulate review_begins
 */
...
void
change_run_review_begin_notify_command(change_ty *cp)
{
    sub_context_ty  *scp;
     ...
     */
    trace(("change_run_review_begin_notify_command(cp = %08lX)\n{\n",
	(long)cp));
    assert(cp->reference_count >= 1);
    the_command = project_review_begin_notify_command_get(cp->pp);
