 ...
 *
 * MANIFEST: functions to manipulate revibegiundos
 */
...
void
change_run_review_begin_undo_notify_command(change_ty *cp)
{
    sub_context_ty  *scp;
     ...
     */
    trace(("change_run_review_begin_undo_notify_command(cp = %08lX)\n{\n",
	(long)cp));
    assert(cp->reference_count >= 1);
    the_command = project_review_begin_undo_notify_command_get(cp->pp);
    if (!the_command || !the_command->str_length)
	goto done;

    /*
     * notify the review begin is rescinded
