...
// Copyright (C) 2000-2008, 2011, 2012 Peter Miller
//
...
#include <libaegis/tstrslt.fmtgen.h>
    ...
    const nstring_list &variable_assignments, const long *remaining)
{
    ...
    for (j = 0; j < wlp->nstrings; ++j)
    {
        ...
        {
            src_data = cp->pp->file_find(fn, view_path_simple);
            assert(src_data);
    ...
    for (j = 0; j < wlp->nstrings; ++j)
    {
        ...
        if (!src_data)
            src_data = cp->pp->file_find(file_name, view_path_simple);
        assert(src_data);
        if (!src_data)
            continue;
