...
{
    trace(("rpt_expr_append(this = %p, ep = %p)\n{\n", this, ep.get()));
...
{
    trace(("rpt_expr::prepend(this = %p, ep = %p)\n{\n", this, ep.get()));
    if (nchild >= nchild_max)
    {
        size_t new_nchild_max = nchild_max * 2 + 4;
        rpt_expr::pointer *new_child = new rpt_expr::pointer [new_nchild_max];
...
rpt_expr::evaluate(bool resolve_deferred, bool deref)
    const
{
    trace(("rpt_expr::execute(this = %p, resdef = %d, deref = %d)\n{\n",
        this, resolve_deferred, deref));
    if (pos)
    {
