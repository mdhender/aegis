...
void
output_cpio_child::changed_size(void)
{
    sub_context_ty sc;
    sc.var_set_format
    (
...
void
output_cpio_child::padding(void)
{
    long n = deeper->ftell();
    assert(n >= 0);
    for (;;)
...
nstring
output_cpio_child::filename(void)
    const
{
...
long
output_cpio_child::ftell_inner(void)
    const
{
    return pos;
}
...
void
output_cpio_child::end_of_line_inner(void)
{
    if (!bol)
        fputc('\n');
}
...
nstring
output_cpio_child::type_name(void)
    const
{
    return ("cpio child > " + deeper->type_name());
}
