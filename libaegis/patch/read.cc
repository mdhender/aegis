...
patch_read(input_ty *input, int required)
{
	...
	string_list_constructor(&garbage);
	for (;;)
	{
		patch_ty	*pp;
		size_t		j;
		...
		//
		trace(("%s\n", input_name(context->input)->str_text));
		pp = 0;
		fp = 0;
		...
		{
			result->description =
				wl2str(&garbage, 0, garbage.nstrings, "\n");
		}
		string_list_destructor(&garbage);
