...
fstate_src_ty *
project::file_find_fuzzy(string_ty *file_name, view_path_ty as_view_path)
{
    //
    // This is used to find names when project::file_find does not.
    // Deleted and almost created files are thus ignored.
    //
    trace(("project::file_find_fuzzy(this = %p, fn = \"%s\")\n{\n",
    ...
    for (size_t j = 0; j < wlp->nstrings; ++j)
    {
        ...
        {
            fstate_src_ty *src_data = file_find(tmp, as_view_path);
            if (src_data)
            {
                best_src = src_data;
                best_weight = weight;
