...
change_list_get(project *pp, time_t limit)
{
    change::pointer pcp;
    cstate_ty       *cstate_data;
    cstate_branch_ty *bp;
    ...
    trace(("change_list_get(pp = %p, limit = %ld \"%.24s\")\n{\n",
        pp, (long)limit, ctime(&limit)));
...
static void
playback_constructor(playback_ty *pbp, time_t limit, project *pp)
{
    trace(("playback_constructor(pbp = %p, limit = %ld \"%.24s\", "
        "pp = %p)\n{\n", pbp, (long)limit, ctime(&limit), pp));
    pbp->pp = pp;
...
{
    trace(("playback_destructor(pbp = %p)\n{\n", pbp));
    if (pbp->clp)
        delete pbp->clp;
    if (pbp->files)
        delete pbp->files;
...
playback_when(playback_ty *pbp)
{
    ...
    return pbp->clp->item[pbp->position]->completion_timestamp();
}
...
{
    trace(("playback_list_constructor(pblp = %p)\n{\n", pblp));
    pblp->length = 0;
    pblp->maximum = 0;
    pblp->item = 0;
    trace(("}\n"));
...
{
    trace(("playback_list_destructor(pblp = %p)\n{\n", pblp));
    if (pblp->item)
    {
        size_t          j;
...
static void
playback_list_push(playback_list_ty *pblp, time_t limit, project *pp)
{
    playback_ty     *pbp;

    trace(("playback_list_push(pblp = %p, limit = %ld \"%.24s\", "
        "pp = %p)\n{\n", pblp, (long)limit, ctime(&limit), pp));
    trace(("project \"%s\"\n", project_name_get(pp).c_str()));
...
{
    trace(("playback_list_pop(pblp = %p)\n{\n", pblp));
...
static void
playback_list_recinit(playback_list_ty *pblp, time_t limit, project *pp)
{
    trace(("playback_list_recinit(pblp = %p, limit = %ld \"%.24s\", "
        "pp = %p)\n{\n", pblp, (long)limit, ctime(&limit), pp));
    while (!pp->is_a_trunk())
    {
        change::pointer cp;
...
playback_list_next(playback_list_ty *pblp, size_t *index_p)
{
    ...
    //
    trace(("playback_list_next(pblp = %p)\n{\n", pblp));
    for (idx = 0; ; ++idx)
    {
        if (idx >= pblp->length)
        {
...
static int
branch_is_completed(project *pp)
{
...
time_t
project_file_roll_forward::recapitulate(project *pp, time_t limit,
    bool detailed)
{
    ...
    trace(("recapitulate(pp = %p, limit = %ld, detailed = %d)\n{\n",
        pp, (long)limit, detailed));
    result = 0;
    ...
    for (;;)
    {
        ...
        trace(("project \"%s\", change %ld, delta %ld\n",
            project_name_get(cp->pp).c_str(),
            magic_zero_decode(cp->number), cp->delta_number_get()));
        last_change = cp;
        ...
        (
            ...
            // at this point.
            //
            cp->completion_timestamp() <= limit
        &&
            (!detailed || !cp->was_a_branch())
        )
        {
            for (j = 0; ; ++j)
            {
                ...
                if (!felp)
                {
                    ...
                    trace(("uuid_to_felp[\"%s\"] = %p\n",
                        vp_src->file_name->str_text, felp.get()));

                    if (vp_src->uuid)
                    {
                        uuid_to_felp.assign(nstring(vp_src->uuid), felp);
                        trace(("uuid_to_felp[\"%s\"] = %p\n",
                            vp_src->uuid->str_text, felp.get()));
                    }
                }
                ...
                &&
                    vp_src->uuid
                &&
                    vp_cp->file_find(nstring(vp_src->move), view_path_first)
                )
                {
                    //
...
void
project_file_roll_forward::set(project *pp, time_t limit, bool detailed)
{
    trace(("project_file_roll_forward(pp = %p, limit = %ld \"%.24s\", "
        "detailed = %d)\n{\n", pp, (long)limit, ctime(&limit), detailed));
...
{
    trace(("project_file_roll_forward::get(%p)\n{\n", src));
...
nstring_list
project_file_roll_forward::get_last_dir(const nstring &dirname)
{
    trace(("project_file_roll_forward::get_last_dir(%s)\n{\n",
        dirname.c_str()));
    nstring_list result;

    nstring_list candidates;
    filename_to_uuid.keys(candidates);
    for (size_t j = 0; j < candidates.size(); ++j)
    {
        nstring candidate = candidates[j];
        if (dirname != ".")
        {
            nstring rhs = os_below_dir(dirname, candidate);
            if (rhs.empty())
                continue;
        }
        file_event *fep = get_last(candidate);
        if (!fep)
            continue;
        fstate_src_ty *src = fep->get_src();

        // this means the file got moved
        if (candidate != nstring(src->file_name))
            continue;
...
nstring
project_file_roll_forward::get_last_fuzzy(const nstring &filename)
{
    trace(("project_file_roll_forward::get_last_fuzzy(%s)\n{\n",
        filename.c_str()));
    nstring result = filename_to_uuid.query_fuzzy(filename);
    trace(("return \"%s\"\n", result.c_str()));
    trace(("}\n"));
    return result;
}
...
project_file_roll_forward::get_older(const nstring &filename)
{
    ...
    for (size_t j = felp->size(); j > 0; --j)
    {
        ...
        time_t when = fep->get_change()->completion_timestamp();
        if (when < stp_time)
        {
            result = fep;
            break;
