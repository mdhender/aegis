...
TEST_SUBJECT="aemakegen, aegis lib/profile"
...
aegis -nf \
        ...
        $work/${AEGIS_PROJECT}.C010/lib/profile \
        $work/${AEGIS_PROJECT}.C010/lib/cshrc \
        $work/${AEGIS_PROJECT}.C010/lib/profile.zsh \
        -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
#
# On a network, this would be unique to each machine. It can be read-only.
#
# The $(DESTDIR) is for packaging.
#
...
$(datadir)/projname/cshrc: .mkdir.__datadir__projname lib/cshrc
{TAB}$(INSTALL_SCRIPT) lib/cshrc $@

$(sysconfdir)/profile.d/projname.csh: .mkdir.__sysconfdir__profile_d lib/cshrc
{TAB}$(INSTALL_SCRIPT) lib/cshrc $@

$(datadir)/projname/profile: .mkdir.__datadir__projname lib/profile
{TAB}$(INSTALL_SCRIPT) lib/profile $@

$(sysconfdir)/profile.d/projname.sh: .mkdir.__sysconfdir__profile_d lib/profile
{TAB}$(INSTALL_SCRIPT) lib/profile $@

$(datadir)/projname/profile.zsh: .mkdir.__datadir__projname lib/profile.zsh
{TAB}$(INSTALL_SCRIPT) lib/profile.zsh $@

$(sysconfdir)/profile.d/projname.zsh: .mkdir.__sysconfdir__profile_d \
{TAB}{TAB}lib/profile.zsh
{TAB}$(INSTALL_SCRIPT) lib/profile.zsh $@
...
.mkdir.__sysconfdir_:
{TAB}-$(INSTALL_DIR) $(sysconfdir)
{TAB}@-test -d $(sysconfdir) && touch $@
{TAB}@sleep 1

.mkdir.__sysconfdir__profile_d: .mkdir.__sysconfdir_
{TAB}-$(INSTALL_DIR) $(sysconfdir)/profile.d
{TAB}@-test -d $(sysconfdir)/profile.d && touch $@
{TAB}@sleep 1
...
{TAB}rm -f .mkdir.__sysconfdir_ .mkdir.__sysconfdir__profile_d core
...
install-datadir: $(datadir)/projname/cshrc $(datadir)/projname/profile \
{TAB}{TAB}$(datadir)/projname/profile.zsh \
{TAB}{TAB}$(sysconfdir)/profile.d/projname.csh \
{TAB}{TAB}$(sysconfdir)/profile.d/projname.sh \
{TAB}{TAB}$(sysconfdir)/profile.d/projname.zsh

uninstall:
{TAB}rm -f $(bindir)/barney $(datadir)/projname/cshrc
{TAB}rm -f $(datadir)/projname/profile $(datadir)/projname/profile.zsh
{TAB}rm -f $(sysconfdir)/profile.d/projname.csh
{TAB}rm -f $(sysconfdir)/profile.d/projname.sh
{TAB}rm -f $(sysconfdir)/profile.d/projname.zsh
...
dist_pkgdatadir_DATA = cshrc profile profile.zsh

# Data files to be installed in $(datarootdir)
dist_datarootdir_DATA = $(sysconfdir)/profile.d/projname.csh \
{TAB}{TAB}$(sysconfdir)/profile.d/projname.sh \
{TAB}{TAB}$(sysconfdir)/profile.d/projname.zsh
...
%{_bindir}/barney
%{_datadir}/projname/cshrc
%{_datadir}/projname/profile
%{_datadir}/projname/profile.zsh
%{_sysconfdir}/profile.d/projname.csh
%{_sysconfdir}/profile.d/projname.sh
%{_sysconfdir}/profile.d/projname.zsh
fubar
if test $? -ne 0 ; then no_result; fi
...
sed "s|{TAB}|${TAB}|g" > ok << 'fubar'
etc/profile.d/projname.csh
etc/profile.d/projname.sh
etc/profile.d/projname.zsh
usr/bin/barney
usr/share/projname/cshrc
usr/share/projname/profile
usr/share/projname/profile.zsh
fubar
if test $? -ne 0 ; then no_result; fi
