...
#
# test the aedist -archive functionality
#
...
#	pippo
#
activity="create the 1st change 193"
cat > caf <<EOF
...
EOF
if test $? -ne 0 ; then no_result; fi
$bin/aegis -nc -f caf -v -p $AEGIS_PROJECT \
     > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
activity="set the UUID 252"
$bin/aegis -ca 10 -v > log 2>&1 \
...
activity="set the UUID 313"
$bin/aegis -ca 11 -v > log 2>&1 \
	--uuid aaaaaaaa-bbbb-4bbb-8ccc-ccccddddddd2
if test $? -ne 0 ; then cat log; no_result; fi

activity="develop end 318"
$bin/aegis -de -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
activity="integrate 326"
$bin/aefinish -p $AEGIS_PROJECT -c 11 > log 2>&1
if test $? -ne 0 ; then cat log ; no_result; fi

activity="generate archive 330"
mkdir $work/archive
if test $? -ne 0; then no_result; fi
$bin/aedist --archive -cd $work/archive > log 2>&1
if test $? -ne 0; then cat log; fail; fi

cat > ok <<EOF
archive/aa.uui/aa.uui/aaaabbbb.4bb/b8cccccc.cdd/ddddd1.ae
archive/aa.uui/aa.uui/aaaabbbb.4bb/b8cccccc.cdd/ddddd1.fp
archive/aa.uui/aa.uui/aaaabbbb.4bb/b8cccccc.cdd/ddddd2.ae
archive/aa.uui/aa.uui/aaaabbbb.4bb/b8cccccc.cdd/ddddd2.fp
EOF
if test $? -ne 0; then no_result; fi

find archive -type f -print | sort > list
if test $? -ne 0; then no_result; fi

diff ok list
if test $? -ne 0; then fail; fi

activity="generate partial archive 350"
mkdir $work/archive.partial
if test $? -ne 0; then no_result; fi

$bin/aedist --archive -cd $work/archive.partial -in-version 1.* > log 2>&1
if test $? -ne 0; then cat log; fail; fi

cat > ok <<EOF
archive.partial/aa.uui/aa.uui/aaaabbbb.4bb/b8cccccc.cdd/ddddd2.ae
archive.partial/aa.uui/aa.uui/aaaabbbb.4bb/b8cccccc.cdd/ddddd2.fp
EOF
if test $? -ne 0; then no_result; fi

find archive.partial -type f -print | sort > list
if test $? -ne 0; then no_result; fi
