...
if test "$AEGIS_SEARCH_PATH" != ""
then
    tpath=
    hold="$IFS"
    IFS=":$IFS"
    for tpath2 in $AEGIS_SEARCH_PATH
    do
...
{
        set +x
        echo "FAILED test of the aelock functionality ($activity)" 1>&2
        cd $here
        find $work -type d -user $USER -exec chmod u+w {} \;
        rm -rf $work
        exit 1
...
{
        set +x
        echo "NO RESULT when testing the aelock functionality ($activity)" 1>&2
        cd $here
        find $work -type d -user $USER -exec chmod u+w {} \;
        rm -rf $work
        exit 2
...
project_specific =
[
    { name = "aelock:command"; value = "aelock -list"; },
];
fubar
if test $? -ne 0 ; then no_result; fi
...
#
# Now create a second change.
# We only want to see if it is present in the lock list.
#
activity="new change 142"
cat > caf << 'fubar'
...
#
# Finally, we get to run the aelock command.
#
$bin/aelock -p $AEGIS_PROJECT command > test.out 2> log
if test $? -ne 0
then
    cat test.out log
    fail
...
n=`wc -l test.out | awk '{print $1}'`
if test $? -ne 0; then fail; fi
if test "$n" -le 4; then cat test.out log; fail; fi
