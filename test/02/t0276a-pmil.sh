...
TEST_SUBJECT="aemakegen, autoconf *.in files"
...
aegis -nf $work/${AEGIS_PROJECT}.C010/aegis.conf \
    ...
    $work/${AEGIS_PROJECT}.C010/fred/something.c.in \
    > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
fred/something.c: config.status fred/something.c.in
{TAB}CONFIG_FILES=$@:fred/something.c.in CONFIG_HEADERS= $(SH) config.status

fred/something.o: fred/something.c
{TAB}$(CC) $(CPPFLAGS) $(CFLAGS) -I. -c fred/something.c
{TAB}mv something.o $@
...
# The fred program.
#
fred_obj = fred/main.o fred/something.o
...
clean-obj:
{TAB}rm -f fred/main.o fred/something.o

distclean: clean distclean-directories distclean-files

distclean-files:
{TAB}rm -f Makefile config.cache config.log config.status fred/something.c
...
activity="aemakegen target=makefile 275"
aemakegen -c 10 --target=makefile > test.out
if test $? -ne 0 ; then fail; fi

activity="check rpm-spec 279"
diff -u ok test.out
if test $? -ne 0 ; then fail; fi
