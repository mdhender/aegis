...
#
# MANIFEST: Test the aeb (optional) functionality
#
...
{
	set +x
	echo "FAILED test of the aeb (optional) functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
{
	set +x
	echo "NO RESULT when testing the aeb (optional) functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 2
...
#
# test the aeb (optional) functionality
#
AEGIS_PATH=$work/lib
export AEGIS_PATH
