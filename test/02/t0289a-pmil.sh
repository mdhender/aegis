...
TEST_SUBJECT="aemakegen, etc/msgfmt.sh"
...
aegis -nf \
        ...
        $work/${AEGIS_PROJECT}.C010/datadir/en/LC_MESSAGES/name.po \
        -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
datadir/en/LC_MESSAGES/name.mo: datadir/en/LC_MESSAGES/name.po etc/msgfmt.sh
{TAB}$(SH) etc/msgfmt.sh --msgfmt=$(MSGFMT) --msgcat=$(MSGCAT) --output=$@ \
{TAB}{TAB}datadir/en/LC_MESSAGES/name.po

$(NLSDIR)/en/LC_MESSAGES/name.mo: .mkdir.__NLSDIR__en_LC_MESSAGES \
{TAB}{TAB}datadir/en/LC_MESSAGES/name.mo
{TAB}$(INSTALL_DATA) datadir/en/LC_MESSAGES/name.mo $@
...
all-i18n: datadir/en/LC_MESSAGES/name.mo
...
clean-i18n:
{TAB}rm -f datadir/en/LC_MESSAGES/name.mo
...
install-i18n: $(NLSDIR)/en/LC_MESSAGES/name.mo

uninstall:
{TAB}rm -f $(NLSDIR)/en/LC_MESSAGES/name.mo $(bindir)/barney
...
data_DATA = $(NLSDIR)/en/LC_MESSAGES/name.mo
...
%{_localedir}/en/LC_MESSAGES/name.mo
%{_bindir}/barney
fubar
if test $? -ne 0 ; then no_result; fi
...
sed "s|{TAB}|${TAB}|g" > ok << 'fubar'
usr/bin/barney
usr/share/locale/en/LC_MESSAGES/name.mo
fubar
if test $? -ne 0 ; then no_result; fi
