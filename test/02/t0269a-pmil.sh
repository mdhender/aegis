...
TEST_SUBJECT="aemakegen, debian vs redundant license files"

# load up standard prelude and test functions
. test_funcs

AEGIS_PROJECT=red-lic-fil
...
aegis -nf $work/${AEGIS_PROJECT}.C010/aegis.conf \
    ...
    $work/${AEGIS_PROJECT}.C010/en/man1/fred.1 \
    $work/${AEGIS_PROJECT}.C010/en/man1/fred-license.1 \
...
cat > $work/${AEGIS_PROJECT}.C010/en/man1/fred.1 << 'fubar'
.XX aaa bbb
fubar
if test $? -ne 0 ; then no_result; fi
...
en/cat1/fred-license.1: en/man1/fred-license.1
{TAB}@mkdir -p en/cat1
{TAB}GROFF_NO_SGR=-c $(GROFF) -I. -Tascii -t -man en/man1/fred-license.1 > $@

$(mandir)/man1/fred-license.1: .mkdir.__mandir__man1 en/man1/fred-license.1
{TAB}$(INSTALL_DATA) en/man1/fred-license.1 $@
...
all-doc: en/cat1/fred-license.1 en/cat1/fred.1
...
clean-doc:
{TAB}rm -f en/cat1/fred-license.1 en/cat1/fred.1
...
install-man: $(mandir)/man1/fred-license.1 $(mandir)/man1/fred.1

uninstall:
{TAB}rm -f $(bindir)/fred $(mandir)/man1/fred-license.1 $(mandir)/man1/fred.1
...
activity="aemakegen 337"
aemakegen -c 10 --target=makefile > ${AEGIS_PROJECT}.C010/Makefile.in
if test $? -ne 0 ; then fail; fi

activity="check makefile 341"
diff -u makefile.ok ${AEGIS_PROJECT}.C010/Makefile.in
if test $? -ne 0 ; then fail; fi

# ----------  debian/{projname}.install  -----------------------------------

activity="aemakegen --target=debian 347"
aemakegen -c 10 --target=debian
if test $? -ne 0 ; then fail; fi

activity="check test.install 351"
cat > ${AEGIS_PROJECT}.install.ok << 'fubar'
usr/bin/fred
usr/share/man/man1/fred.1
fubar
if test $? -ne 0 ; then no_result; fi

diff -u ${AEGIS_PROJECT}.install.ok \
        ${AEGIS_PROJECT}.C010/debian/${AEGIS_PROJECT}.install
if test $? -ne 0 ; then fail; fi

# ----------  debian/rules  ------------------------------------------------

activity="check rules 364"
...
{TAB}{TAB}--mandir=/usr/share/man CFLAGS="$(CFLAGS)" LDFLAGS="-Wl,-z,defs"
...
{TAB}-rm -f $(CURDIR)/debian/tmp/usr/share/man/man1/fred-license.1
...
diff -u rules.ok ${AEGIS_PROJECT}.C010/debian/rules
if test $? -ne 0 ; then fail; fi
