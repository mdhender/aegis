...
{
        set +x
        echo "FAILED test of the aecp -delta ($activity)" 1>&2
        cd $here
        find $work -type d -user $USER -exec chmod u+w {} \;
        rm -rf $work
        exit 1
...
{
        set +x
        echo "NO RESULT when testing the aecp -delta functionality " \
	    "($activity)" 1>&2
        cd $here
        find $work -type d -user $USER -exec chmod u+w {} \;
        rm -rf $work
...
#	aegis.conf
#
activity="create the 1st change 205"
cat > caf <<EOF
...
activity="integrate 271"
$bin/aefinish -p $AEGIS_PROJECT -c 10 -v > log 2>&1
if test $? -ne 0 ; then cat log ; no_result; fi

activity="remove pfs 275"
rm $work/proj.dir/info/change/0/001.branch/0/001.branch/0/010.pfs
if test $? -ne 0; then cat log; no_result; fi
...
activity="modify a file 298"
$bin/aegis -cp $workchan/pippo -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

cat >> $workchan/pippo <<EOF
Add some not so random text
EOF
if test $? -ne 0; then no_result; fi
...
activity="finish the change 335"
$bin/aefinish -p test.1 1 -v > log 2>&1
if test $? -ne 0 ; then cat log ; no_result; fi

activity="new branch 339"
...
activity="integrate begin 387"
$bin/aegis -ib -c 11 -v > log 2>&1
if test $? -ne 0 ; then cat log ; no_result; fi

activity="integrate 391"
$bin/aefinish -p $AEGIS_PROJECT -c 11 > log 2>&1
if test $? -ne 0 ; then cat log ; no_result; fi
...
activity="develop begin 410"
workchan=$work/change-dir-12
$bin/aegis -db 12 -dir $workchan -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi

activity="modify pluto 415"
$bin/aegis -cp $workchan/pluto -v -nl > log 2>&1
if test $? -ne 0; then no_result; fi
...
activity="integrate begin 436"
$bin/aegis -ib -c 12 -v > log 2>&1
if test $? -ne 0 ; then cat log ; no_result; fi

activity="integrate 440"
...
activity="develop end 465"
$bin/aefinish 13 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="integrate begin 469"
$bin/aegis -ib 13 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="diff the change 473"
$bin/aegis -diff 13 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="finish change 477"
$bin/aefinish 13 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="complete branch 481"
$bin/aegis -de -p test.1 2 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="integrate begin 485"
$bin/aegis -ib -p test.1 2 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="diff the change 489"
$bin/aegis -diff -p test.1 2 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="build the change 493"
$bin/aegis -build -p test.1 2 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="integrate pass 497"
$bin/aegis -ipass -p test.1 2 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="new branch 501"
$bin/aegis -nbr -p test.1 3 -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi
...
activity="develop begin 529"
workchan=$work/change-dir-99-10
$bin/aegis -db 10 -dir $workchan -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi

activity="copy files 534"
$bin/aegis -cp -br 1.3 -delta 0 $workchan -nl -v > log 2>&1
if test $? -ne 0; then cat log; no_result; fi

activity="check change contents 538"
cat > ok <<EOF
config modify 2 aegis.conf
source remove 5 pluto
EOF
if test $? -ne 0; then no_result; fi

$bin/aegis -l cf -p $AEGIS_PROJECT -c 10 -unf > list.out 2> log
if test $? -ne 0; then cat log; no_result; fi
