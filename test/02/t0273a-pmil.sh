...
TEST_SUBJECT="aemakegen, non-etc/test.sh tests"

# load up standard prelude and test functions
. test_funcs

AEGIS_PROJECT=no-etc-test-sh
...
activity="aemakegen target=makefile 285"
aemakegen -c 10 --target=makefile > test.out
if test $? -ne 0 ; then fail; fi

activity="check makefile 289"
diff -u ok test.out
if test $? -ne 0 ; then fail; fi

# ----------  target=automake  ---------------------------------------------

activity="aemakegen --target=automake 295"
aemakegen -c 10 --target=automake > test.out
if test $? -ne 0 ; then fail; fi

activity="check automake 299"
cat > ok << 'fubar'
#
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#
