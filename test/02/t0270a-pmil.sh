...
TEST_SUBJECT="aemakegen, yacc vs makefile"
...
aegis -nf $work/${AEGIS_PROJECT}.C010/aegis.conf \
    ...
    $work/${AEGIS_PROJECT}.C010/fred/example.y \
...
fred/example.yacc.c fred/example.yacc.h: fred/example.y
{TAB}$(YACC) -d $(YFLAGS) fred/example.y
{TAB}sed -e 's/[yY][yY]/example_/g' -e '/<stdio.h>/d' -e '/<stdlib.h>/d' -e \
{TAB}{TAB}'/<stddef.h>/d' y.tab.c > fred/example.yacc.c
{TAB}sed -e 's/[yY][yY]/example_/g' -e 's/Y_TAB_H/fred_example_YACC_H/g' \
{TAB}{TAB}y.tab.h > fred/example.yacc.h
{TAB}test -f y.output && mv y.output fred/example.output || true
{TAB}rm -f y.tab.c y.tab.h y.output || true

fred/example.yacc.o: fred/example.yacc.c
{TAB}$(CC) $(CPPFLAGS) $(CFLAGS) -I. -c fred/example.yacc.c
{TAB}mv example.yacc.o $@
...
# The fred program.
#
fred_obj = fred/example.yacc.o fred/main.o
...
clean-obj:
{TAB}rm -f fred/example.list fred/example.yacc.c fred/example.yacc.h
{TAB}rm -f fred/example.yacc.o fred/main.o y.output y.tab.c y.tab.h
...
activity="aemakegen target=makefile 306"
aemakegen -c 10 --target=makefile > test.out
if test $? -ne 0 ; then fail; fi

activity="check makefile 310"
diff -u ok test.out
if test $? -ne 0 ; then fail; fi

# ----------  target=automake  ---------------------------------------------

activity="aemakegen --target=automake 316"
aemakegen -c 10 --target=automake > test.out
if test $? -ne 0 ; then fail; fi

activity="check automake 320"
cat > ok << 'fubar'
#
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#
...
bin_PROGRAMS = bin/fred

# The fred program.
bin_fred_includes = fred/example.yacc.h
bin_fred_SOURCES = fred/example.y fred/main.c
...
BUILT_SOURCES = fred/example.yacc.c fred/example.yacc.h
