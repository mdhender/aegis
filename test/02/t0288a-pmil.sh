...
TEST_SUBJECT="aemakegen, aegis' lib/* files"

# load up standard prelude and test functions
. test_funcs

AEGIS_PROJECT=alibtest
...
aegis -nf \
        ...
        $work/${AEGIS_PROJECT}.C010/lib/icon.png \
        $work/${AEGIS_PROJECT}.C010/lib/examples/fred \
        -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
$(datadir)/alibtest/examples/fred: .mkdir.__datadir__alibtest_examples \
{TAB}{TAB}lib/examples/fred
{TAB}$(INSTALL_DATA) lib/examples/fred $@

$(datadir)/alibtest/icon.png: .mkdir.__datadir__alibtest lib/icon.png
{TAB}$(INSTALL_DATA) lib/icon.png $@
...
.mkdir.__datadir__alibtest: .mkdir.__datadir_
{TAB}-$(INSTALL_DIR) $(datadir)/alibtest
{TAB}@-test -d $(datadir)/alibtest && touch $@
{TAB}@sleep 1

.mkdir.__datadir__alibtest_examples: .mkdir.__datadir__alibtest
{TAB}-$(INSTALL_DIR) $(datadir)/alibtest/examples
{TAB}@-test -d $(datadir)/alibtest/examples && touch $@
{TAB}@sleep 1
...
clean-misc:
{TAB}rm -f .bin .mkdir.__bindir_ .mkdir.__datadir_ .mkdir.__datadir__alibtest
{TAB}rm -f .mkdir.__datadir__alibtest_examples core
...
install-datadir: $(datadir)/alibtest/examples/fred $(datadir)/alibtest/icon.png

uninstall:
{TAB}rm -f $(bindir)/barney $(datadir)/alibtest/examples/fred
{TAB}rm -f $(datadir)/alibtest/icon.png
...
# Data files to be installed in $(datadir)/alibtest/
dist_pkgdatadir_DATA = examples/fred icon.png

# Data to be installed below $(datarootdir)/
data_DATA = lib/examples/fred lib/icon.png
...
Source: http://alibtest.sourceforge.net/%{name}-%{version}.tar.gz
URL: http://alibtest.sourceforge.net/
...
%{_bindir}/barney
%{_datadir}/alibtest/examples/fred
%{_datadir}/alibtest/icon.png
fubar
if test $? -ne 0 ; then no_result; fi
...
activity="aemakegen 455"
aemakegen -c 10 --target=debian --flavour=aegis
if test $? -ne 0 ; then fail; fi

activity="check debian/alibtest.install 459"
sed "s|{TAB}|${TAB}|g" > ok << 'fubar'
usr/bin/barney
usr/share/alibtest/examples/fred
usr/share/alibtest/icon.png
fubar
if test $? -ne 0 ; then no_result; fi

diff -u ok $work/${AEGIS_PROJECT}.C010/debian/alibtest.install
if test $? -ne 0 ; then fail; fi
