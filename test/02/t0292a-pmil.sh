...
TEST_SUBJECT="aemakegen, aegis' test_funcs"
...
aegis -nf \
        ...
        $work/${AEGIS_PROJECT}.C010/script/test_funcs.in \
        -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi

cat > $work/${AEGIS_PROJECT}.C010/script/test_funcs.in << 'fubar'
#!/bin/sh
exit 0
fubar
if test $? -ne 0 ; then cat log; no_result; fi
...
script/test_funcs: config.status script/test_funcs.in
{TAB}CONFIG_FILES=$@:script/test_funcs.in CONFIG_HEADERS= $(SH) config.status

bin/test_funcs: .bin script/test_funcs
{TAB}cp script/test_funcs $@
{TAB}chmod a+rx $@
...
all-bin: bin/barney bin/test_funcs
...
clean-bin:
{TAB}rm -f bin/barney bin/test_funcs
...
distclean-files:
{TAB}rm -f Makefile config.cache config.log config.status script/test_funcs
...
{TAB}{TAB}distclean-files install install-bin sure t0001a \
...
bin/test_funcs: script/test_funcs
{TAB}@mkdir -p bin
{TAB}cp script/test_funcs $@
{TAB}chmod a+rx $@
...
noinst_SCRIPTS = bin/test_funcs

# scripts needed to run tests
check_SCRIPTS = bin/test_funcs

# Files to be removed by the "distclean" make target.
DISTCLEANFILES = script/test_funcs
...
EXTRA_DIST = aegis.conf script/test_funcs.in

# Catch-22 when building dependencies.
BUILT_SOURCES = script/test_funcs
...
activity="aemakegen 454"
aemakegen -c 10 --target=debian --flavour=aegis
if test $? -ne 0 ; then fail; fi

activity="check debian/projname.install 458"
sed "s|{TAB}|${TAB}|g" > ok << 'fubar'
usr/bin/barney
fubar
if test $? -ne 0 ; then no_result; fi
