...
#
# MANIFEST: Test the aesvt get_head functionality
#
...
{
	set +x
	echo "FAILED test of the aesvt get_head functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
{
	set +x
	echo "NO RESULT when testing the aesvt get_head functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 2
...
$bin/aesvt -hist test,svt -q > test.out
test $? -eq 0 || fail
