...
#
# MANIFEST: Test aemtu functionality
#
...
{
	set +x
	echo "NO RESULT for test of aemtu functionality ($activity)" 1>&2
	cd $here
...
{
	set +x
	echo "FAILED test of aemtu functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
cat > $tmp << 'end'
description = "A bogus project created to test the aemtu functionality.";
...
# now remove it again
#
activity="make transparent undo 428"
$bin/aegis -mtu $workchan/bogus -nl > log 2>&1
if test $? -ne 0 ; then cat log; fail; fi

activity="check change file state 432"
cat > ok << 'end'
src =
[
];
