...
#
# MANIFEST: Test the aerbu functionality
#
...
{
	set +x
	echo NO RESULT for test of aerbu functionality "($activity)" 1>&2
	cd $here
...
{
	set +x
	echo FAILED test of aerbu functionality "($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
