...
#	Copyright (C) 1999, 2005-2007 Peter Miller
#
...
#
# MANIFEST: Test the add_path_suffix subst functionality
#
...
{
	set +x
	echo 'FAILED test of the add_path_suffix subst functionality' 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
{
	set +x
	echo 'NO RESULT when testing the add_path_suffix subst functionality' 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 2
...
#
# test the add_path_suffix subst functionality
#
...
$bin/aegis -nf $work/chan/aegis.conf -c 10 -p test -lib $work/lib -v > LOG 2>&1
if test $? -ne 0 ; then cat LOG; no_result; fi
...
[
	{
		pattern = [ "*" ];
		body = "${add-path-suffix /bin /:/usr:/usr/local}";
	}
];
fubar
if test $? -ne 0 ; then no_result; fi
