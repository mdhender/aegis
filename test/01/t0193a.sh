...
{
        set +x
        echo 'FAILED test of the aecvsserver remove functionality' \
                "($activity)" 1>&2
        cd $here
        find $work -type d -user $USER -exec chmod u+w {} \;
        rm -rf $work
...
{
        set +x
        echo 'NO RESULT when testing the aecvsserver remove functionality' \
                "($activity)" 1>&2
        cd $here
        find $work -type d -user $USER -exec chmod u+w {} \;
        rm -rf $work
...
activity="set the change UUID 264"
$bin/aegis -ca -uuid aaaaaaaa-bbbb-4bbb-8ccc-ccccddddddd5 > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
cvs -d :fork:/aegis remove bogus > log 2>&1
