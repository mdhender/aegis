...
#
# MANIFEST: Test the shell_safe_filenames functionality
#
...
{
	set +x
	echo "FAILED test of the shell_safe_filenames functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
{
	set +x
	echo "NO RESULT for test of the shell_safe_filenames functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 2
