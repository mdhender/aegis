...
#	Copyright (C) 1998, 2000, 2004-2007 Peter Miller
#
...
#
# MANIFEST: Test the quote subst functionality
#
...
{
	set +x
	echo "FAILED test of the quote subst functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
{
	set +x
	echo "NO RESULT for test of the quote subst functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 2
...
#
# test the quote subst functionality
#
...
$bin/aegis -nf "$work/test.C010/test(a)" -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
