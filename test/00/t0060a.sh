...
#	Copyright (C) 1997, 1998, 2000, 2004-2007 Peter Miller
#
...
#
# MANIFEST: Test the aecp -indep functionality
#
...
{
	set +x
	echo 'NO RESULT for test of the aecp -indep functionality' 1>&2
	cd $here
...
{
	set +x
	echo 'FAILED test of the aecp -indep functionality' 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
#
# test the aecp -indep functionality
#
...
activity="integrate build 270"
$bin/aegis -b -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi

activity="integrate diff 274"
$bin/aegis -diff -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
#
# now copy independently
#
activity="aecp -independent 285"
mkdir $work/i1
if test $? -ne 0 ; then no_result; fi
cd $work/i1
if test $? -ne 0 ; then no_result; fi
$bin/aegis -cp -indep . -v > log 2>&1
if test $? -ne 0 ; then cat log; fail; fi
diff $work/test/baseline/fred fred
if test $? -ne 0 ; then fail; fi
diff $work/test/baseline/aegis.conf aegis.conf
if test $? -ne 0 ; then fail; fi
diff $work/test/baseline/test/00/t0001a.sh test/00/t0001a.sh
if test $? -ne 0 ; then fail; fi

#
# now copy independently with delta
#
activity="aecp -independent -delta 302"
mkdir $work/i2
if test $? -ne 0 ; then no_result; fi
cd $work/i2
if test $? -ne 0 ; then no_result; fi
$bin/aegis -cp -indep . -v -delta 1 > log 2>&1
if test $? -ne 0 ; then cat log; fail; fi
