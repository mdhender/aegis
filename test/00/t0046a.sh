...
#
# MANIFEST: Test the aerm directory functionality
#
...
{
	set +x
	echo "FAILED test of the aerm directory functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
{
	set +x
	echo "NO RESULT for  test of the aerm directory functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 2
