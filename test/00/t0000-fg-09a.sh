...
private:
    /**
    * The fred instance variable is used to remember the boolean fred.
    */
    bool fred: 1;

    /**
    * The fred_is_set_flag instance variable is used to remember whether or
    * not the #fred instance variable has been set.
    */
    bool fred_is_set_flag: 1;
...
test::test(const test &rhs) :
    fred(rhs.fred),
    fred_is_set_flag(rhs.fred_is_set_flag)
{
    trace(("%s\n", __PRETTY_FUNCTION__));
}
...
test::fred_is_set()
    const
{
    return (fred || fred_is_set_flag);
}

void
test::fred_set(bool x)
{
    fred = x;
    fred_is_set_flag = true;
}
...
test::fred_introspector_factory()
{
    return introspector_boolean_by_method < test > ::create ( *this,
        &test::fred_set, &test::fred_is_set);
}
...
{
    trace(("%s\n{\n", __PRETTY_FUNCTION__));
    introspector_boolean::write(fp, "fred", fred, fred_is_set_flag);
    trace(("}\n"));
}
    ...
    const
{
    trace(("%s\n{\n", __PRETTY_FUNCTION__));
    fp->fputs("<test>\n");
    introspector_boolean::write_xml(fp, "fred", fred, fred_is_set_flag);
    fp->fputs("</test>\n");
    trace(("}\n"));
}
