...
#
# MANIFEST: Test the project_file_dir function
#
...
{
	set +x
	echo "FAILED test of the project_file_dir function ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
{
	set +x
	echo "NO RESULT for test of the project_file_dir function ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 2
...
#
# test the project_file_dir function
#
...
$bin/aegis -nf $work/test.1.0.C010/aegis.conf $work/test.1.0.C010/fred/fred -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
fubar
if test $? -ne 0 ; then no_result; fi
echo hello > $work/test.1.0.C010/fred/fred
if test $? -ne 0 ; then no_result; fi
