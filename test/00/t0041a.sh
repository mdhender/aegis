...
#
# MANIFEST: Test the aecpu directory functionality
#
...
{
	set +x
	echo "FAILED test of the aecpu directory functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 1
...
{
	set +x
	echo "NO RESULT for test of the aecpu directory functionality ($activity)" 1>&2
	cd $here
	find $work -type d -user $USER -exec chmod u+w {} \;
	rm -rf $work
	exit 2
...
#
# test the aecpu directory functionality
#
...
activity="copy file 228"
$bin/aegis -cp $workchan/c/d $workchan/c/e > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
activity="copy dir undo 261"
$bin/aegis -cpu $workproj/baseline/c -v > log 2>&1
if test $? -ne 0 ; then cat log; fail; fi

activity="check change file state 265"
cat > ok << 'fubar'
src =
[
	{
