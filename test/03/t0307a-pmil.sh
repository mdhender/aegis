...
TEST_SUBJECT='aemakegen, vs $(datarootdir)'
...
aegis -nf \
        ...
        $work/${AEGIS_PROJECT}.C010/datarootdir/blah-blah-blah \
        -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
$(datarootdir)/blah-blah-blah: .mkdir.__datarootdir_ datarootdir/blah-blah-blah
{TAB}$(INSTALL_DATA) datarootdir/blah-blah-blah $@
...
clean-misc:
{TAB}rm -f .bin .mkdir.__bindir_ .mkdir.__datarootdir_ core
...
install-datadir: $(datarootdir)/blah-blah-blah

uninstall:
{TAB}rm -f $(bindir)/example $(datarootdir)/blah-blah-blah
