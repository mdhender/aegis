...
TEST_SUBJECT='aemakegen, vs $(libdir)'
...
aegis -nf \
        ...
        $work/${AEGIS_PROJECT}.C010/libdir/pkgconfig/fred.pc.in \
        -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
libdir/pkgconfig/fred.pc: config.status libdir/pkgconfig/fred.pc.in
{TAB}CONFIG_FILES=$@:libdir/pkgconfig/fred.pc.in CONFIG_HEADERS= $(SH) \
{TAB}{TAB}config.status

$(libdir)/pkgconfig/fred.pc: .mkdir.__libdir__pkgconfig libdir/pkgconfig/fred.pc
{TAB}$(INSTALL_DATA) libdir/pkgconfig/fred.pc $@
...
{TAB}rm -f libdir/pkgconfig/fred.pc
...
install-libdir: $(libdir)/pkgconfig/fred.pc

uninstall:
{TAB}rm -f $(bindir)/example $(libdir)/pkgconfig/fred.pc
