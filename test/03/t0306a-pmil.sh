...
aegis -nf \
        ...
        $work/${AEGIS_PROJECT}.C010/libdir/pic.gif \
        -v > log 2>&1
if test $? -ne 0 ; then cat log; no_result; fi
...
$(libdir)/pic.gif: .mkdir.__libdir_ libdir/pic.gif
{TAB}$(INSTALL_DATA) libdir/pic.gif $@
...
clean-misc:
{TAB}rm -f .bin .mkdir.__bindir_ .mkdir.__libdir_ core
...
install-libdir: $(libdir)/pic.gif

uninstall:
{TAB}rm -f $(bindir)/example $(libdir)/pic.gif
